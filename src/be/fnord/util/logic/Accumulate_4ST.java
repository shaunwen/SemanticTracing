...
public class Accumulate_4ST extends Accumulate {
	...
	/**
	 * Accumulate the effects for a given trace from a BPMN model under certain
	 * knowledge base
	 * 
	 * @param src
	 *            the input trace
	 ...
	 *            knowledge base
	 * @param gST
	 *            the graph for storing semantic vertices and edges
	 * @return gST the graph with effect scenarios and the corresponding links
	 *         stored
	 */
	public Graph_ST<Vertex_ST, Edge_ST> trace_acc(
		Trace src, String kb, Graph_ST<Vertex_ST, Edge_ST> gST) {

		// The following vList is a 2-dimension array for storing the candidate
		// vertices for the graph
		ArrayList<ArrayList<Vertex_ST>> vList =
			new ArrayList<ArrayList<Vertex_ST>>();

		LinkedHashSet<WFF> currentEff = new LinkedHashSet<WFF>(); // current
																	// effect
		...
		for (Vertex v : src.getNodes()) { // traverse nodes in the given trace
			...
			for (WFF ce : currentEff) {
				if (!v.getWFF().isEmpty()) {
					intEff.remove(ce);
					Accumulate_4ST acc = new Accumulate_4ST(); // Needed because
																// of
			...
			// if it is not the first/last Vertex, then store the Vertex to the
			// Graph, then Edge
			if (src.getNodes().get(src.getNodes().size() - 1).equals(v) ||
				src.getNodes().get(0).equals(v)) {
				continue; // don't print the last accumulative effect, it would
							// be a repetition;
							// don't print the first accumulative effect
							// neither, because it is empty
			}
			else {
				for (WFF e : intEff) {
					// System.out.println(e);
					// gST.addVertex(new Vertex_ST(v.name,
					// v.getWFF().toString(), e.toString()) );
					tmpV.add(new Vertex_ST(
						v.name, v.getWFF().toString(), e.toString())); // add
																		// candidate
																		// vertex
																		// to
																		// ArrayList
		...
		for (int i = 0; i < vList.size(); i++) {
			for (int j = 0; j < vList.get(i).size(); j++) {
				...
				// vList.get(i).get(j).taskName);
				// System.out.println("immediate effect: " +
				// vList.get(i).get(j).immWFF);
				// System.out.println("cummulative effect: " +
				// vList.get(i).get(j).esWFF);
				// System.out.println("---End a node in the graph----\n");
			}
			...
			for (int j = 0; j < vList.get(i).size(); j++) {
				for (int x = 0; x < vList.get(i - 1).size(); x++) {
					...
					if (containsES(effCheckList, vList.get(i).get(j).esWFF)) {
						...
						// System.out.println("####edge: " + vSource.esWFF +
						// " linked to " + vTarget.esWFF);
	...
	/**
	 * Check if the effect scenario of target Vertex is in the accumulated
	 * effect set, if it is in the checkList, then return true, otherwise,
	 * return false.
	 * 
	 * @param checkList
	 *            the list with effect scenarios
	 * @param vTarget_esWFF
	 *            the given effect scenarios
	 * @return true / false
	 */
